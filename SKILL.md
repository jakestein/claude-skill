...
All three fields are required. The new user is created as an admin of the new org. The response includes `api_key` (the production-bucket key the agent uses from here on), `user_id`, and `organization_id`. Two emails are also sent to the new user's address: One is a verification email and the other is a password reset email.
